function [f,s,m,mv,mvd,unA] = expmv(t,A,b,M,prec,shiftmat,bal,full_term,prnt,force_estm,tol)
...
% Construct ExpmvArray object
if ~isa( A, 'ExpmvArray' )
    A	=	ExpmvArray( A, size(A), prec );
end
...
if nargin < 4 || isempty(M)
   tt = 1;
   if t == 1, At = A; else At = t.*A; end
   [M,mvd,alpha,unA] = select_taylor_degree(At,[],[],prec,false,false,force_estm);
   clear At
   mv = mvd;
else
   tt = t; mv = 0; mvd = 0;
