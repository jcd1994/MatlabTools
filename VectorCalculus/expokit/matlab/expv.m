...
% make A a function handle and get inf norm -JD
if ~isa(A,'function_handle')
    anorm = norm(A,'inf');
    A = @(x) A*x;
else
    % Estimate Matrix infinity norm (for near sparse matrices) -JD
%     anorm = -inf;
%     count = 0;
%     while count < 5
%         anorm0 = max( A( 2*(rand(n,1)>0.5)-1 ) );
%         if anorm0 > anorm
%             anorm = anorm0;
%             count = 0;
%         else
...
while t_now < t_out
    ...
    for j = 1:m
        % p = A*V(:,j);
        p = A(V(:,j)); % function handle change -JD
        for i = 1:j
            H(i,j) = V(:,i)'*p;
            p = p-H(i,j)*V(:,i);
        end;
    ...
    if k1 ~= 0,
        H(m+2,m+1) = 1;
        % avnorm = norm(A*V(:,m+1));
        avnorm = norm(A(V(:,m+1)));  % function handle change -JD
    end;
    ireject = 0;
    while ireject <= mxrej,
        mx = mb + k1;
